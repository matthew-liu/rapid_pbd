...
int main(int argc, char** argv) {
  ...
  if (argc < 6) {
    ROS_ERROR("Must supply rgb, depth, cam_info, cloud topics and template_dir as args");
    return 1;
  }
  ...
  std::string template_dir(argv[5]);

  ROS_INFO("template dir: %s", template_dir.c_str());
  ...
  rapid::pbd::FindLandmark2DAction action(cam_info_topic, cloud_topic, scene_db, *robot_config, template_dir);
